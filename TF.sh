...
for i in $(seq 1 $(stty size | cut -d' ' -f2)); do echo -n "-"; done
wget -O- -q http://1337x.unbanthe.org/sort-search/${SEA// /%20}/seeders/desc/1/ | grep '<strong><a href="/torrent' | cut -d '"' -f2 | sed 's\/\http://1337x.unbanthe.org/\' | head -5 | xargs wget -O- -q | grep 'href="http://torcache.net/torrent/' | cut -d '"' -f4 | cat -n
for i in $(seq 1 $(stty size | cut -d' ' -f2)); do echo -n "="; done
...
for i in $(seq 1 $(stty size | cut -d' ' -f2)); do echo -n "-"; done
wget -O- -q http://iso.unbanthe.org/torrents/\?ihq\=${SEA// /+}\&Torrent_sort\=seeders.desc | grep '"title-row"><a href="' | cut -d '"' -f12 | sed 's\/\http://iso.unbanthe.org/\' | head -5 |  xargs wget -O- -q | grep '<a href="https://torrent.isohunt.to/download.php' | cut -d '"' -f2 | cat -n
for i in $(seq 1 $(stty size | cut -d' ' -f2)); do echo -n "="; done
